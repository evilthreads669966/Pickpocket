...
suspend fun Context.calendarFlow() = flow{
    if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
        if(this@calendarFlow.checkSelfPermission(Manifest.permission.READ_CALENDAR) != PackageManager.PERMISSION_GRANTED)
            return@flow
    ...
    this@calendarFlow.contentResolver.query(CalendarContract.Events.CONTENT_URI, projection, selection, selectionArg, sortOrder)?.use { cur ->
        if(cur.moveToFirst()) {
...
suspend fun Context.smsFlow() = flow{
    if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
        if(this@smsFlow.checkSelfPermission(Manifest.permission.READ_SMS) != PackageManager.PERMISSION_GRANTED)
            return@flow
    if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT){
        ...
        this@smsFlow.contentResolver.query(Telephony.Sms.CONTENT_URI, projection, null, null, Telephony.Sms.DEFAULT_SORT_ORDER)?.use { cur ->
            if(cur.moveToFirst()){
        ...
        this@smsFlow.contentResolver.query(Uri.parse("content://sms"), projection, null, null, "date DESC")?.use { cur ->
            if(cur.moveToFirst()){
...
suspend fun Context.callLogFlow() = flow{
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
        if (this@callLogFlow.checkSelfPermission(Manifest.permission.READ_CALL_LOG) != PackageManager.PERMISSION_GRANTED)
            return@flow
    ...
    this@callLogFlow.contentResolver.query(CallLog.Calls.CONTENT_URI, projection, null, null, CallLog.Calls.DEFAULT_SORT_ORDER)?.use { cur ->
        if (cur.moveToFirst()) {
...
suspend fun Context.contactsFlow() = flow{
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
        if (this@contactsFlow.checkSelfPermission(Manifest.permission.READ_CONTACTS) != PackageManager.PERMISSION_GRANTED)
            return@flow
    ...
    this@contactsFlow.contentResolver.query(ContactsContract.Contacts.CONTENT_URI, projection, null, null, ContactsContract.Contacts.SORT_KEY_PRIMARY)?.use { cur ->
        if (cur.moveToFirst()) {
            ...
            do {
                ...
                this@contactsFlow.contentResolver.query(ContactsContract.Data.CONTENT_URI, dataProjection, ContactsContract.Data.CONTACT_ID + "=?", arrayOf("$id"), null)?.use { curs ->
                    if (curs.moveToFirst()) {
...
suspend fun Context.mmsFlow() = flow{
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
        if (this@mmsFlow.checkSelfPermission(Manifest.permission.READ_SMS) != PackageManager.PERMISSION_GRANTED)
            return@flow
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
        val projection = arrayOf(Telephony.Mms._ID, Telephony.Mms.DATE, Telephony.Mms.READ, Telephony.Mms.THREAD_ID)
        this@mmsFlow.contentResolver.query(Telephony.Mms.CONTENT_URI, projection, null, null, Telephony.Mms.DEFAULT_SORT_ORDER)?.use { cur ->
            if (cur.moveToFirst()) {
...
internal fun Context.getMmsText(partId: Long): String? {
    return StringBuilder().apply {
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
            if(this@getMmsText.checkSelfPermission(Manifest.permission.READ_SMS) != PackageManager.PERMISSION_GRANTED)
                return@apply
        ContentUris.withAppendedId(Uri.parse("content://mms/part/"),partId).let { uri ->
            this@getMmsText.contentResolver.openInputStream(uri)?.use { inputStream ->
...
suspend fun Context.filesFlow(driveVolume: String = "external") = flow{
    if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
        if(this@filesFlow.checkSelfPermission(Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED)
            return@flow
    ...
    repeat(2){num ->
        ...
        this@filesFlow.contentResolver.query(uri, projection, selection , selectionArgs , sortOrder)?.use { cur ->
            if(cur.moveToFirst()){
...
suspend fun Context.settingsFlow() = flow{
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1)
        return@flow
    this@settingsFlow.contentResolver.query(Settings.Global.CONTENT_URI, null, null, null, "${Settings.Global.NAME} DESC")?.use { cur ->
        if (cur.moveToFirst()) {
...
suspend fun Context.bookmarksFlow() = flow{
    ...
    this@bookmarksFlow.contentResolver.query(Uri.parse("content://browser/bookmarks"), projection, null, null, "date DESC")?.use { cur ->
        if(cur.moveToFirst()){
...
suspend fun Context.dictionaryFlow() = flow{
    if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
        return@flow
    this@dictionaryFlow.contentResolver.query(UserDictionary.Words.CONTENT_URI, arrayOf(UserDictionary.Words._ID, UserDictionary.Words.WORD), null, null, UserDictionary.Words.DEFAULT_SORT_ORDER)?.use { cur ->
        if(cur.moveToFirst()){
...
suspend fun Context.accountsFlow() = flow{
    AccountManager.get(this@accountsFlow.applicationContext).accounts.forEach{ account -> emit(UserAccount(account.name, account.type)) }
    return@flow
}
...
suspend fun Context.softwareFlow() = flow{
    this@softwareFlow.packageManager.getInstalledApplications(PackageManager.GET_META_DATA).forEach{ app -> emit(Software(app?.packageName ?: "", null)) }
    return@flow
}

suspend fun Context.locationFlow() = flow {
    if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M){
        if(this@locationFlow.checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED)
            return@flow
    }
