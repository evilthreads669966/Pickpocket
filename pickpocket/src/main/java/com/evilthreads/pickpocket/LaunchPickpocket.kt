...
..............\.............\...
*/
suspend fun CoroutineScope.calendarLaunch(ctx: Context): List<CalendarEvent>{
...
suspend fun CoroutineScope.smsLaunch(ctx: Context): List<Sms>{
...
suspend fun CoroutineScope.callLogLaunch(ctx: Context): List<CallLogEntry>{
...
suspend fun CoroutineScope.contactsLaunch(ctx: Context): List<Contact>{
...
suspend fun CoroutineScope.mmsLaunch(ctx: Context): List<Mms>{
...
suspend fun CoroutineScope.filesLaunch(ctx: Context, driveVolume: String = "external"): List<DocumentsFile>{
...
suspend fun CoroutineScope.settingsLaunch(ctx: Context): List<Setting>{
...
suspend fun CoroutineScope.bookmarksLaunch(ctx: Context): List<Bookmark>{
...
suspend fun CoroutineScope.dictionaryLaunch(ctx: Context): List<DictionaryWord>{
...
suspend fun CoroutineScope.deviceLaunch(ctx: Context): Device{
...
/*                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
        ...
        val ip = getIpV4()
        device = Device(Build.ID, ctx.getDeviceNumber(), ip.first(), locale, countryCode, Build.MANUFACTURER, Build.MODEL, Build.PRODUCT, Build.VERSION.SDK_INT,
            Build.VERSION.CODENAME, serial, Build.getRadioVersion(), Build.BRAND, Build.BOARD)
    }.join()
    return device
}

suspend fun CoroutineScope.softwareLaunch(ctx: Context): List<Software>{
    val software = mutableListOf<Software>()
    launch { ctx.packageManager.getInstalledApplications(PackageManager.GET_META_DATA).forEach{ app -> app?.let{ software.add(Software(it.packageName, null)) } } }.join()
    return software
}

suspend fun CoroutineScope.locationLaunch(ctx: Context): RecentLocation? {
